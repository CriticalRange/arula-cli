...
impl<'a, Message> LivingBackground<'a, Message> {
    pub fn new(state: &'a LivingBackgroundState, palette: PaletteColors, opacity: f32) -> Self {
        Self { state, palette, opacity, _marker: PhantomData }
    }
}
    ...
    fn draw(
        ...
        let living = self.state.cache.draw(renderer, bounds.size(), |frame| {
            ...
            let project = |x: f32, y: f32, z: f32| -> Option<iced::Point> {
                if z <= 1.0 { return None; } // Behind camera
                let scale = fov / z;
            ...
            frame.with_save(|frame| {
                ...
                for y_level in [-200.0, 200.0] { 
                    ...
                    for i in -15..=15 {
                        ...
                        if let (Some(p1), Some(p2)) = (project(x_pos, y_level, z_start), project(x_pos, y_level, z_end)) {
                            let stroke = canvas::Stroke {
                                style: canvas::Style::Solid(Color { a: 0.15 * self.opacity, ..self.palette.accent }),
                                width: 1.0,
                    ...
                    for i in 0..num_lines {
                        ...
                        if z < 10.0 { continue; } // Too close/behind
                        ...
                        if let (Some(p1), Some(p2)) = (project(-width, y_level, z), project(width, y_level, z)) {
                            let stroke = canvas::Stroke {
                                style: canvas::Style::Solid(Color { a: alpha, ..self.palette.accent }),
